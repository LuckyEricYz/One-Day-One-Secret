...
import { getShanghaiDateKey } from "./time.js";
...
function getCurrentSolarTerm(timestamp: number) {
  ...
  if (target < boundaries[0].date) {
    current = {
      ...
      date: createBoundaryDate(
        year - 1,
        SOLAR_TERMS[SOLAR_TERMS.length - 1].month,
        SOLAR_TERMS[SOLAR_TERMS.length - 1].day
      )
    };
    next = boundaries[0];
  }
...
export function getCalendarContext(timestamp: number): CalendarContext {
  const year = Number(getShanghaiDateKey(timestamp).slice(0, 4));
  const lichun = createBoundaryDate(year, 2, 4);
